...
tag_identity() {
  [[ "${DOCPRIMS_TAGGER_NAME:-} <${DOCPRIMS_TAGGER_EMAIL:-}>" == "$(cat "$tag_root/config/release/tagger-identity.txt")" ]] || {
    tag_die 'infosec tagger identity required'
    return 1
  }
  [[ "$DOCPRIMS_TAGGER_NAME" != *$'\n'* && "$DOCPRIMS_TAGGER_NAME" != *$'\r'* &&
    "$DOCPRIMS_TAGGER_NAME" != *'<'* && "$DOCPRIMS_TAGGER_NAME" != *'>'* ]] || {
    tag_die 'invalid tagger name'
    return 1
  }
  [[ "$DOCPRIMS_TAGGER_EMAIL" != *$'\n'* && "$DOCPRIMS_TAGGER_EMAIL" != *$'\r'* &&
    "$DOCPRIMS_TAGGER_EMAIL" != *'<'* && "$DOCPRIMS_TAGGER_EMAIL" != *'>'* ]] || {
...
tag_version() {
  [[ -n "${DOCPRIMS_RELEASE_TAG:-}" ]] || {
    tag_die 'DOCPRIMS_RELEASE_TAG required'
    return 1
  }
...
tag_message_file() {
  [[ -n "${DOCPRIMS_TAG_MESSAGE_DIR:-}" && -d "$DOCPRIMS_TAG_MESSAGE_DIR" &&
    ! -L "$DOCPRIMS_TAG_MESSAGE_DIR" ]] || {
  ...
  local canonical
  canonical="$(cd "$DOCPRIMS_TAG_MESSAGE_DIR" && pwd -P)"
  case "$canonical" in "$tag_root" | "$tag_root"/*)
    tag_die 'message directory must be outside repository'
    return 1
    ;;
  esac
  local file="${DOCPRIMS_TAG_MESSAGE_DIR%/}/message.txt"
...
tag_origin() {
  ...
  case "$url" in
  https://github.com/3leaps/docprims | https://github.com/3leaps/docprims.git | \
    git@github.com:3leaps/docprims | git@github.com:3leaps/docprims.git) ;;
  git@*:3leaps/docprims | git@*:3leaps/docprims.git)
    host="${url#git@}"
    host="${host%%:*}"
    ...
    ;;
  *)
    tag_die 'origin must be 3leaps/docprims on GitHub'
    return 1
    ;;
  esac
}
...
tag_selector_shape() {
  [[ "${DOCPRIMS_PGP_KEY_ID:-}" =~ ^[0-9A-F]{40}!$ ]] || {
    tag_die 'exact uppercase signing subkey fingerprint with ! required'
    return 1
  }
  [[ "${DOCPRIMS_GPG_SIGNING_FINGERPRINT:-}" =~ ^[0-9A-F]{40}$ ]] || {
...
tag_key_selector() {
  ...
  [[ "$(awk '$1=="gpg" {print $2}' "$tag_root/keys/expected-fingerprints.txt")" == "$DOCPRIMS_GPG_SIGNING_FINGERPRINT" ]] || {
    tag_die 'operator fingerprint differs from committed anchor'
    return 1
  }
  [[ -n "${DOCPRIMS_GPG_HOMEDIR:-}" && -d "$DOCPRIMS_GPG_HOMEDIR" ]] || {
    tag_die 'external GPG home required'
    return 1
  }
  local home
  home="$(cd "$DOCPRIMS_GPG_HOMEDIR" && pwd -P)"
  ...
  listing="$(gpg --batch --with-colons --fingerprint --with-subkey-fingerprint --list-keys "${DOCPRIMS_PGP_KEY_ID%!}" 2>/dev/null)" || tag_die 'signing subkey unavailable'
  primary="$(printf '%s\n' "$listing" | awk -F: '$1=="pub" {p=1;next} p && $1=="fpr" {print $10;exit}')"
  subkey="$(printf '%s\n' "$listing" | awk -F: -v f="${DOCPRIMS_PGP_KEY_ID%!}" '$1=="sub" {s=1;cap=$12;next} s && $1=="fpr" {if ($10==f && cap ~ /[sS]/) print $10; s=0}')"
  [[ "$primary" == "$DOCPRIMS_GPG_SIGNING_FINGERPRINT" && "$subkey" == "${DOCPRIMS_PGP_KEY_ID%!}" ]] || {
...
tag_verify_object() {
  ...
  [[ "$(git cat-file tag "$object" | sed -n 's/^tag //p' | head -1)" == "$DOCPRIMS_RELEASE_TAG" ]] || {
  ...
  [[ "$tagger" == "$DOCPRIMS_TAGGER_NAME <$DOCPRIMS_TAGGER_EMAIL>" ]] || {
