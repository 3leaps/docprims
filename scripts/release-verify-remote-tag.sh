...
local_object="$(git rev-parse "refs/tags/$DOCPRIMS_RELEASE_TAG")"
remote_object="$(git ls-remote origin "refs/tags/$DOCPRIMS_RELEASE_TAG" | awk '{print $1}')"
[[ -n "$remote_object" && "$remote_object" == "$local_object" ]] || tag_die 'remote tag object differs from local tag'
tag_ref="$(gh api "repos/3leaps/docprims/git/ref/tags/$DOCPRIMS_RELEASE_TAG")"
[[ "$(jq -r '.object.type' <<<"$tag_ref")" == tag && "$(jq -r '.object.sha' <<<"$tag_ref")" == "$local_object" ]] || tag_die 'GitHub tag ref mismatch'
tag_json="$(gh api "repos/3leaps/docprims/git/tags/$local_object")"
jq -e --arg commit "$(git rev-parse HEAD)" --arg tag "$DOCPRIMS_RELEASE_TAG" \
