...
: "${DOCPRIMS_MINISIGN_PUB:?public minisign export required}"
...
"$RELEASE_DECERNOR_BIN" fingerprint "$DOCPRIMS_MINISIGN_PUB" --class public --kind minisign \
...
for kind in gpg minisign; do
  "$RELEASE_DECERNOR_BIN" validate --schema "$root/schemas/release/fingerprint-record.v0.schema.json" \
...
"$root/scripts/install-release-anchors.sh" "$scratch" "$root/keys" \
  ...
  --gpg "$root/docs/security/release-signing-keys.asc" --minisign "$DOCPRIMS_MINISIGN_PUB" >/dev/null
