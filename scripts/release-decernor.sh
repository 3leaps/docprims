...
resolve_release_decernor() {
  ...
  if [[ "$mode" == ceremony ]]; then
    [[ -n "${DOCPRIMS_DECERNOR_BIN:-}" ]] || {
      echo 'error: DOCPRIMS_DECERNOR_BIN is required for the release ceremony' >&2
      return 1
    }
    candidate="$DOCPRIMS_DECERNOR_BIN"
  else
    candidate="${DOCPRIMS_DECERNOR_BIN:-${DECERNOR_BIN:-}}"
    if [[ -z "$candidate" ]]; then
