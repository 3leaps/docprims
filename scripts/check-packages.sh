...
package_target=$(mktemp -d "$temp_root/docprims-package.XXXXXX")
...
# Packaging the crates together resolves their dependencies on each other
# from the packaged sources, as crates.io will after an ordered publish.
cargo package "${package_args[@]}" --locked

package_dir="$CARGO_TARGET_DIR/package"
actual=()
while IFS= read -r archive; do
  actual+=("$(basename "$archive")")
done < <(find "$package_dir" -maxdepth 1 -type f -name 'docprims-*.crate' | sort)
...
def check_dependencies(sections):
    for section in ("dependencies", "dev-dependencies", "build-dependencies"):
        for name, spec in sections.get(section, {}).items():
            if name == "docprims" or name.startswith("docprims-"):
