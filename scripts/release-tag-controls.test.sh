...
if DOCPRIMS_TEST_FAIL_ANCHOR_INSTALL=1 "$root/scripts/install-release-anchors.sh" \
...
export DOCPRIMS_RELEASE_TAG=v1.2.3
export DOCPRIMS_TAG_MESSAGE_DIR="$scratch/v1.2.3"
export DOCPRIMS_TAGGER_NAME='3 Leaps Infosec Team'
export DOCPRIMS_TAGGER_EMAIL='infosec@3leaps.net'
...
expect_fail tag_message_file
printf 'Release v1.2.3\n' >"$scratch/v1.2.3/message.txt"
expect_fail env DOCPRIMS_TAGGER_NAME=$'bad\nname' bash -c 'source "$1"; tag_identity' _ "$root/scripts/release-tag-common.sh"
expect_fail env DOCPRIMS_TAGGER_EMAIL='a<b>' bash -c 'source "$1"; tag_identity' _ "$root/scripts/release-tag-common.sh"
expect_fail env -u DOCPRIMS_TAG_MESSAGE_DIR bash -c 'source "$1"; tag_message_file' _ "$root/scripts/release-tag-common.sh"
expect_fail env DOCPRIMS_TAG_MESSAGE_DIR="$scratch" bash -c 'source "$1"; tag_message_file' _ "$root/scripts/release-tag-common.sh"
export DOCPRIMS_PGP_KEY_ID=0123456789ABCDEF
expect_fail tag_selector_shape
unset DOCPRIMS_PGP_KEY_ID
expect_fail tag_selector_shape
...
export DOCPRIMS_PGP_KEY_ID=0123456789ABCDEF0123456789ABCDEF01234567!
(
  cd "$scratch/repo"
...
{"name":"Tag Publish Protection","source_type":"Repository","source":"3leaps/docprims","target":"tag","enforcement":"active","conditions":{"ref_name":{"exclude":[],"include":["refs/tags/v*"]}},"rules":[{"type":"creation"},{"type":"deletion"},{"type":"non_fast_forward"},{"type":"update"}],"bypass_actors":[{"actor_id":null,"actor_type":"OrganizationAdmin","bypass_mode":"always"}]}
JSON
...
(
  cd "$scratch/repo"
  GIT_COMMITTER_NAME="$DOCPRIMS_TAGGER_NAME" GIT_COMMITTER_EMAIL="$DOCPRIMS_TAGGER_EMAIL" \
