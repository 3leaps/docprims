...
case "$mode" in
before)
  releases="$(mktemp "${TMPDIR:-/tmp}/docprims-release-list.XXXXXX")"
  trap 'rm -f "$releases"' EXIT
  gh release list --repo "$DOCPRIMS_REPOSITORY" --limit 100 \
    --json tagName >"$releases"
  if jq -e --arg tag "$tag" \
    'any(.[]; .tagName == $tag)' "$releases" >/dev/null; then
    assert_github_release_state release_base_assets
  else
    [[ "$(gh repo view "$DOCPRIMS_REPOSITORY" \
      --json nameWithOwner --jq .nameWithOwner)" == "$DOCPRIMS_REPOSITORY" ]] || {
