...
grep -Fq 'NODE_VERSION="$(node -p' "$workflow" ||
  fail "workflow must read the active Node runtime version"
grep -Fq 'version_ge "${NODE_VERSION}" "${MIN_TRUSTED_PUBLISH_NODE}"' "$workflow" ||
