...
fixture="$(mktemp -d "${TMPDIR:-/tmp}/docprims-release-guard.XXXXXX")"
remote="$(mktemp -d "${TMPDIR:-/tmp}/docprims-release-remote.XXXXXX")"
...
git -C "$fixture" remote add origin https://github.com/3leaps/docprims.git
git -C "$fixture" config "url.$remote.insteadOf" \
  https://github.com/3leaps/docprims.git
git -C "$fixture" push -q origin main refs/tags/v1.2.3

run_guard() {
  (
    cd "$fixture"
    env -u DOCPRIMS_RELEASE_TAG \
      -u DOCPRIMS_REQUIRE_TAG \
      -u DOCPRIMS_RELEASE_KEY \
      -u RELEASE_TAG \
      "$@"
  )
}
...
run_guard DOCPRIMS_RELEASE_TAG=v1.2.3 ./guard.sh >/dev/null
for invalid in 1.2.3 vv1.2.3 "v1.2.3 " v1.2.3-rc.1 v1.2.3+build; do
  expect_fail run_guard DOCPRIMS_RELEASE_TAG="$invalid" ./guard.sh
done
expect_fail run_guard DOCPRIMS_RELEASE_TAG=v1.2.4 ./guard.sh
expect_fail run_guard RELEASE_TAG=v1.2.3 ./guard.sh
expect_fail run_guard DOCPRIMS_RELEASE_KEY=v1.2.3 ./guard.sh
