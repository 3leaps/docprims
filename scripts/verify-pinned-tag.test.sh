...
(
  ...
  GIT_COMMITTER_NAME='3 Leaps Infosec Team' GIT_COMMITTER_EMAIL=infosec@3leaps.net \
    git tag -s -a --cleanup=verbatim -u "$subkey!" -F "$scratch/message.txt" "$DOCPRIMS_RELEASE_TAG"
)
...
verify >/dev/null
DOCPRIMS_PGP_KEY_ID="$subkey!" verify >/dev/null
...
(
  cd "$fixture"
  GIT_COMMITTER_NAME='3 Leaps Infosec Team' GIT_COMMITTER_EMAIL=infosec@3leaps.net \
    git tag -fs -a --cleanup=verbatim -u "$subkey!" -F "$scratch/message.txt" "$DOCPRIMS_RELEASE_TAG"
  tag_verify_object refs/tags/v1.2.3 "$scratch/message.txt"
)
verify >/dev/null
printf 'Fixture release\n' >"$scratch/message.txt"
...
(
  cd "$fixture"
  GIT_COMMITTER_NAME='3 Leaps Infosec Team' GIT_COMMITTER_EMAIL=infosec@3leaps.net \
    git tag -fs -a --cleanup=verbatim -u "$primary!" -F "$scratch/message.txt" "$DOCPRIMS_RELEASE_TAG"
)
expect_fail verify
(
  cd "$fixture"
...
verify >/dev/null
git -C "$fixture" tag -d "$DOCPRIMS_RELEASE_TAG" >/dev/null
git -C "$fixture" tag "$DOCPRIMS_RELEASE_TAG"
expect_fail verify
