...
main() {
  ...
  if [[ -n "${RELEASE_TAG+x}" || -n "${DOCPRIMS_RELEASE_KEY+x}" ]]; then
  ...
  expected="v${version}"
  tag="${DOCPRIMS_RELEASE_TAG:-$expected}"
  require_tag="${DOCPRIMS_REQUIRE_TAG:-0}"
  ...
  if [[ "$require_tag" == "1" ]]; then
    if [[ -z "${DOCPRIMS_RELEASE_TAG:-}" ]]; then
      echo "error: strict release guard requires DOCPRIMS_RELEASE_TAG" >&2
      exit 1
    fi
    ...
    case "$origin_url" in
    https://github.com/3leaps/docprims | \
    ...
    git@*:3leaps/docprims | git@*:3leaps/docprims.git)
      ...
      ;;
    *)
      echo "error: origin must identify github.com/3leaps/docprims" >&2
      exit 1
      ;;
    esac
