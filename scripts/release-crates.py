...
# Workspace crates that must never reach crates.io.
UNPUBLISHED = ("docprims-ffi", "docprims-ts-napi")
...
def validate(packages, names, manifests):
    if (
        ...
        or any(n != "docprims" and not n.startswith("docprims-") for n in names)
    ):
